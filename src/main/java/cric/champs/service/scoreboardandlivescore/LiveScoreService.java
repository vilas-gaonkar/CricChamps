...
public class LiveScoreService implements LiveInterface {
    ...
    private void updateBatsmanSBForLegBye(LiveScoreUpdate liveScoreUpdateModel, Long scoreBoardId) {
        ...
        doStrikeRotationForLegBye(strikePosition, liveScoreUpdateModel.getStrikeBatsmanId(), scoreBoardId, 1);
        if (strikePosition.equals(StrikePosition.STRIKE.toString()))
            doStrikeRotationForLegBye(StrikePosition.NONSTRIKE.toString(), liveScoreUpdateModel.getNonStrikeBatsmanId(),
                    scoreBoardId, 0);
        else
            doStrikeRotationForLegBye(StrikePosition.STRIKE.toString(), liveScoreUpdateModel.getNonStrikeBatsmanId(),
                    scoreBoardId, 0);
    }

    private void doStrikeRotationForLegBye(String strikePosition, Long batsmanId, Long scoreBoardId, int ball) {
        jdbcTemplate.update("update batsmanSB set balls = balls + ? , strikePosition = ? where scoreBoardId = ? " +
                "and playerId = ?", ball, strikePosition, scoreBoardId, batsmanId);
    }
